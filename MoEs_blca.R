...
MoE_s<-function (Y, X, G=3, delta = 1, a=1, b=0.005,v=0.01, nseg=20, bdeg=3, burn.in=0.1, maxiter = 5000, seed=303) {
	...
	if(G>1){
		...
		Wstore<-array(0, dim=c(maxiter,N,G))		
		groupstore<-matrix(0,N,maxiter)
		S<-matrix(0,N,G) 
		...
		for (it in 1:maxiter) {
			...
			groupstore[,it]<-apply(S,1,which.max)
	...
	if(G==1){
		...
		finish = proc.time()[3]
		duration = finish - starttime
		output<-list(theta=colMeans(thetastore[(burn.in+1):maxiter,]),
			#AICM=aicm(llik[(burn.in+1):maxiter]),
			AICM=-2*(mean(llik[(burn.in+1):maxiter])-var(llik[(burn.in+1):maxiter])),
			BICM=-2*(mean(llik[(burn.in+1):maxiter])-log(N-1)*var(llik[(burn.in+1):maxiter])),
			DIC=-2*(2*mean(llik[(burn.in+1):maxiter])-sum(log(rowMeans(lik[,(burn.in+1):maxiter])))),
